...
}> = ({ open, onClose, cart, articles, onConfirm }) => {
      ...
      {step === 2 && (
        <div className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button variant="filled" icon="check_circle" onClick={() => onConfirm(form)}>Confirmar pedido</Button>
          </div>
        </div>
      )}
    </Modal>
...
const StoreView: React.FC = () => {
  const [articles] = useLocalStorage<Article[]>('plantarte_articles', []);
  const [, setStoreOrders] = useLocalStorage<StoreOrder[]>('plantarte_store_orders', []);
  ...
  const handleConfirm = (form: CheckoutForm) => {
